  ...
  Stream<TimerState> mapEventToState(
    ...
    if (event is TimerStarted) {
      yield* _mapTimerStartedToState(event);
       ...
       yield* _mapTimerTicketToState(event);
...
Stream<TimerState> _mapTimerResetToState(TimerReset reset) async* {
  ...
  Stream<TimerState> _mapTimerTicketToState(TimerTicked tick) async* {
  yield TimerRunState(tick.timeStr, tick.percent);
}

  static String _calculationTime(int currentWaitInSec) {
    var minuteStr = (currentWaitInSec ~/ 60).toString().padLeft(2, '0');
    var secondStr = (currentWaitInSec % 60).toString().padLeft(2, '0');
